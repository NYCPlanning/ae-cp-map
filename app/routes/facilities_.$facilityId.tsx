...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const facilityCategoriesPromise = findFacilityCategories({
    baseURL: `${zoningApiUrl}/api`,
  });

  const [facility, facilityAgencies, facilityCategories] = await Promise.all([
    facilityPromise,
  ...
  for (const category of facilityCategories) {
    for (const group of category.groups) {
      for (const subgroup of group.subgroups) {
        facilityCategoriesMap.set(
          `${category.id}-${group.id}-${subgroup.id}`,
          `${category.name} / ${group.name} / ${subgroup.name}`,
        );
      }
    }
  }
...
export default function Facility() {
  ...
  const categoryPath = facilityCategoriesMap.get(
    `${facility.categoryId}-${facility.categoryGroupId}-${facility.categorySubgroupId}`,
  );
  ...
  return (
    <ContentPanelAccordion accordionHeading="Details">
      <FacilityDetail
        facility={facility}
        categoryPath={
          categoryPath === undefined ? "Not specified" : categoryPath
        }
